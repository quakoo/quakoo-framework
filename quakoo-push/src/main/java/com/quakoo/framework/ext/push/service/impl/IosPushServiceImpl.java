	...
	public void batchPush(List<PushUserInfoPool> userInfos, Payload payload) {
		...
		for(Entry<Long, List<String>> entry : uidTokenMap.entrySet()) {
            ...
            try {
                ...
                customFields.put("id", String.valueOf(payload.getId()));
                if (null != payload.getExtra() && payload.getExtra().size() > 0) {
                    customFields.putAll(payload.getExtra());
                }
                ...
                logger.error("===========ios token : "+ tokens.toString() + " payload : " + payload.getTitle());
            } catch (Exception e) {
	...
	public void push(PushUserInfoPool userInfo, Payload payload) {
		...
		if(verifyToken(token)) {
			...
			try {
                ...
                logger.error("===========ios token : "+ token + " payload : " + payload.getTitle());
			} catch (Exception e) {
				cache.decr(String.format(badgeKeyFormat, uid));
				logger.error(e.getMessage(), e);
			}
