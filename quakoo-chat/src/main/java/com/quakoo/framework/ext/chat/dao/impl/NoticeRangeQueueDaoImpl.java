...
public class NoticeRangeQueueDaoImpl extends BaseDaoHandle implements NoticeRangeQueueDao, InitializingBean {

    private final static String notice_range_queue_key = "%s_notice_range_queue";
...
//	private final static String notice_range_queue_status_null_key = "%s_notice_range_queue_status_%d_null";
    ...
    public boolean insert(NoticeRangeQueue one) throws DataAccessException {
        String queue_key = String.format(notice_range_queue_key, chatInfo.projectName);
        long time = one.getTime();
        queueClient.zaddObject(queue_key, new Double(time), one);
        return true;
    }
