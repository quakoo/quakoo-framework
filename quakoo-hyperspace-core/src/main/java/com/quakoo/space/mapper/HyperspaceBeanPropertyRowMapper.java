 ...
 */
public class HyperspaceBeanPropertyRowMapper<T> implements RowMapper<T> {
	...
	private Map<String, Type> jsonFields=new HashMap<>();
	 ...
	 */
	public HyperspaceBeanPropertyRowMapper() {
	}
	 ...
	 */
	public HyperspaceBeanPropertyRowMapper(Class<T> mappedClass) throws Exception {
	 ...
	 */
	public HyperspaceBeanPropertyRowMapper(Class<T> mappedClass, boolean checkFullyPopulated) throws Exception {
		initialize(mappedClass);
	...
	protected void initialize(Class<T> mappedClass) throws Exception {
		...
		for (PropertyDescriptor pd : pds) {
			if (pd.getWriteMethod() != null) {
				...
				Field field = ReflectUtil.getFieldByName(pd.getName(), mappedClass);
				HyperspaceColumn autowareMap = field.getAnnotation(HyperspaceColumn.class);
				if(autowareMap!=null&&autowareMap.isJson()){
					jsonFields.put(lowerCaseName(pd.getName()),field.getGenericType());
				}
	 ...
	 * @param mappedClass the class that each row should be mapped to
	 */
	public static <T> HyperspaceBeanPropertyRowMapper<T> newInstance(Class<T> mappedClass) throws Exception {
		return new HyperspaceBeanPropertyRowMapper<>(mappedClass);
	}
